...
import { Eye, EyeOff, Wallet, ShieldCheck, Lock, TrendingUp } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col lg:flex-row bg-[var(--bg-color)] text-[var(--text-color)] font-sans">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 md:p-24 bg-[var(--bg-color)] relative">
        <div className="w-full max-w-md space-y-8 relative z-10">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight mb-2 text-[var(--text-color)]">Create account</h1>
            <p className="text-gray-500 dark:text-gray-400 font-medium text-sm">Start building healthy money habits today.</p>
          </div>

          <form onSubmit={onSubmit} className="space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 ml-1">Full Name</label>
              <input
                type="text"
            ...
            <div className="pt-4">
              ...
              >
                {authLoading ? 'Creating account...' : (
                   <>Create Account <Lock size={16} strokeWidth={2.5} /></>
                )}
              </button>
            </div>
          </form>
          ...
          <div className="mt-8 pt-8 border-t border-gray-100 dark:border-white/5">
            <p className="text-sm text-center font-medium text-gray-500 dark:text-gray-400">
              Already have an account? <Link to="/login" className="text-emerald-500 font-bold hover:text-emerald-600 transition-colors ml-1">Login here</Link>
            </p>
          </div>
        </div>
      </div>
